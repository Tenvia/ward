...
| API unit test baseline | prototype, demo-supported | `apps/api/src/*.test.ts` covers tenant state transitions, detection thresholds, approval issuance/phrase/TTL/single-use, and audit append/list. Determinstic, no Docker, no provider calls. Verified by `npm run test:unit` (4 suites, 27 checks, all pass). |
