...
| Pull-based Compose file (prepared) | prototype (inactive until publish) | `docker-compose.pull.yml` mirrors the user bundle against `ghcr.io/jenksed/ward-api:v0.1.0-rc1` (WARD_IMAGE-overridable); config validates. Will work only after the first image publish. |
