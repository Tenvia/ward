...
| Pull-based Compose file (prepared) | prototype (inactive until publish) | `docker-compose.pull.yml` mirrors the user bundle against `ghcr.io/10via/ward-api:latest`; config validates. Will work only after the first image publish. |
